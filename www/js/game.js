class GameScene extends Phaser.Scene {
    ...
    preload() {
        this.load.spritesheet('bird', 'img/bird.png', { frameWidth: BIRD_SIZE, frameHeight: BIRD_SIZE });
    ...
    create() {
        ...
        this.score = 0;
        this.labelScore = this.add.text(20, 20, '0', { font: TEXT_SIZE + 'px Arial', fill: '#ffffff' }).setOrigin(0, 0);
        ...
        this.bird.body.setSize(BIRD_SIZE - 15, BIRD_SIZE - 15);
    ...
    update() {
        if (this.bird.body.y < -this.bird.height / 2 || this.bird.y > GAME.config.height + this.bird.height / 2) this.restart();
    ...
    addOnePipe(x, y, typePipe) {
        ...
        this.pipes.add(pipe);
        pipe.body.setVelocityX(-200 * SCALING_FACTOR);
        pipe.body.setAllowGravity(false);
        pipe.setDepth(0);
    }

    addRowofPipes() {
        const COLUMN_SIZE = Math.ceil(GAME.config.height / (PIPE_SIZE));
        var hole = Math.floor(Math.random() * (COLUMN_SIZE - 4)) + 1;
        for (var i = 0; i < COLUMN_SIZE; i++) {
            var pipeType = 'pipe';
            if (i != hole && i != hole + 1) {
                if (i == hole - 1) pipeType = 'pipe-up';
                else if (i == hole + 2) pipeType = 'pipe-down';
                this.addOnePipe(400 * SCALING_FACTOR, i * PIPE_SIZE, pipeType);
            }
        }
        this.score++;
    ...
    jump() {
        if (this.bird.alive) {
            this.bird.body.velocity.y = -350 * SCALING_FACTOR;
